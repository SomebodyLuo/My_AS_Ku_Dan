...
// luoyouren-20180131: 12. user set callback
int UVCPreview::setFrameCallback(JNIEnv *env, jobject frame_callback_obj, int pixel_format) {
	...
	pthread_mutex_lock(&capture_mutex);
	{
		...
		if (frame_callback_obj) {
			// luoyouren-20180201: the format of frame getting by callback
...
// luoyouren-20180201: get callback adapt to format
...
//
//**********************************************************************
// luoyouren-20180131: 5. callback will call addPreviewFrame()
...
// luoyouren-20180131: add frame for another thread by mutex
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
	...
	// luoyouren-20180131: 4.set callback
	...
	if (LIKELY(!result)) {
		...
		if (frameMode) {
			...
			for ( ; LIKELY(isRunning()) ; ) {
			    ...
			    // luoyouren-20180131: 6. addPreviewFrame() wake this
				...
				if (LIKELY(frame_mjpeg)) {
					...
					if (LIKELY(!result)) {
					    ...
					    // luoyouren-20180131: 7. do surface preview
						frame = draw_preview_one(frame, &mPreviewWindow, uvc_any2rgbx, 4);

						// luoyouren-20180131: 8. add frame for capture, will wake capture thread!!!
 ...
 */
 // luoyouren-20180131: get frame from another thread by mutex
...
void UVCPreview::do_capture_surface(JNIEnv *env) {
	...
	for (; isRunning() && isCapturing() ;) {
	    // luoyouren-20180131: 8. wake by addCaptureFrame() and get frame
		...
		if (LIKELY(frame)) {
			...
			// luoyouren-20180131: 9. java user capture callback with frame
		...
		if (mFrameCallbackObj) {
			if (mFrameCallbackFunc) {
				...
				if (LIKELY(callback_frame)) {
					...
					// luoyouren-20180131: 10. format convert
			...
			// luoyouren-20180131: 11. call user's java callback function
