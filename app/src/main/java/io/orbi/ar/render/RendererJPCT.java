...
//	public static class Point { public double x,y; }
	...
	public RendererJPCT(Context a) { context=a; AssetJPCT.context=a; AssetJPCT.texMgr=texMgr; initAll(); }
...
//			ShadowJPCT.shdHelp.blit(fb);
	...
	class ScreenManager { int width,height,halfWidth,halfHeight; double aspect;
	...
	static void loadTex(int a, String b, Texture c, boolean d) {
		...
		texName[a]=b; tex[a]=c; tex[a].compress();
		tex[a].setTextureCompression(d);
		texMgr.addTexture(texName[a],tex[a]); }

	static void unloadTex(int a) { if (tex[a]!=null) texMgr.unloadTexture(fb,tex[a]); }

	void gainFocus() { RttJPCT.gainFocus(); G3dJPCT.gainFocus();
		EnvironmentJPCT.gainFocus(); ViewportJPCT.gainFocus(); ShadowJPCT.gainFocus(); }

	void initAll() { initConfig(); MathJPCT.init();
		RttJPCT.init(); G3dJPCT.init(); EnvironmentJPCT.init(); ShadowJPCT.init(); }

	void initConfig() { Config.glIgnoreNearPlane=false; Config.glTrilinear=true;
		Config.glTransparencyMul=0.004f; Config.glTransparencyOffset=0;
		Config.specPow=90; Logger.setLogLevel(0); }

	public void init(Object3D a)
	{
		if (a.getName()=="md2sample") { md2sample=a; G3dJPCT.addObj(2); }
		else if (a.getName()=="spacecraft1") { spacecraft1=a; G3dJPCT.addObj(3); }
		else if (a.getName()=="plant01") { plant01=a; G3dJPCT.addObj(4); }
		else { soccerball=a; G3dJPCT.addObj(1); }
		G3dJPCT.setObjVisible(0,true);
	}
	public void setFovX(float a) { if (screenMgr!=null) ViewportJPCT.setFovX(a/2); }
	public void setElevation(float a) {}
	public void setSunMode(boolean a) { EnvironmentJPCT.enableSun(a); }
	public void setSunLum(float a) { EnvironmentJPCT.setSunRGB(a/255,a/255,a/255); }
	public void setAmbientLum(float a) { EnvironmentJPCT.setAmbientRGB(a/255,a/255,a/255); }
	public void setOrientationProvider(OrientationProvider a) { orientProvider=a; }
	public void setVisible(boolean a) { G3dJPCT.visObjs[0].show(a); }
	public boolean objectVisible() { return (!G3dJPCT.visObjs[0].hidden); }
	public void setUserScaleFactor(float a) { userScale=a; }
	public int addSun() { return EnvironmentJPCT.addLight(true); }
	...
	public void updatePosition(Point a,float b,float c,float d,double e,double f) {
		...
		float[] m,n=new float[3],v=ViewportJPCT.fovTan; float s;
...
//		m=new float[]{0.707f,0,-0.707f,0,0,0,0,0,0.707f,0,0.707f};
		...
		G3dJPCT.setObjPos(0,n,null); objPosMode=0;
...
//		m=new float[]{0.707f,0,-0.707f,0,0,0,0,0,0.707f,0,0.707f};
	...
	private void stickObj() { if (objPosMode==0) return;
		if (objPosMode==1) stickObjMode1(); else stickObjMode2(); }

	private void stickObjMode1() { float[] u=refPos,n=vec9tmp2,v=ViewportJPCT.fovTan;
		n[1]=(float)Math.atan(pos2ref[0]*v[0]); n[0]=(float)Math.atan(pos2ref[1]*v[1]);
		MathJPCT.phr2vec9_f(n,vec9tmp1); MathJPCT.vec9add(vec9tmp1,ViewportJPCT.vec9,n);
		if (n[2]>=0) return;
		G3dJPCT.setObjPos(0,new float[]{u[2]*n[0]/n[2],u[2]*n[1]/n[2],u[2]},null); }

	private void stickObjMode2() { float[] u=refPos,v=pos2ref;
		G3dJPCT.setObjPos(0,new float[]{u[0]+v[0],u[1]+v[1],u[2]},null); }

	public void stick2screen(float a,float b) { objPosMode=1; pos2ref[0]=1-2*a; pos2ref[1]=2*b-1; }

	public void stick2plane(float a,float b) { float[] n=vec9tmp2,v=ViewportJPCT.fovTan;
		n[1]=(float)Math.atan((1-2*a)*v[0]); n[0]=(float)Math.atan((2*b-1)*v[1]);
		MathJPCT.phr2vec9_f(n,vec9tmp1); MathJPCT.vec9add(vec9tmp1,ViewportJPCT.vec9,n);
		if (n[2]>=0) return; objPosMode=2;
		pos2ref[0]=refPos[2]*n[0]/n[2]-refPos[0]; pos2ref[1]=refPos[2]*n[1]/n[2]-refPos[1]; }
