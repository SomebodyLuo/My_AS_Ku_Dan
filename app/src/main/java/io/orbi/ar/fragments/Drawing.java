...
class Drawing {
    ...
    private static class Quadrilateral {
        ...
        static
        {
            ...
            mPathPaint.setStrokeWidth(2);//set line sIze
     ...
     */
    static void drawPrimitive(Canvas canvas, Matrix transform, DrawingPrimitive primitive, Point p0, Point p1, Point p2, Point p3, String label,Bitmap bmp,boolean debug) {
        ...
        else if (primitive == DrawingPrimitive.DRAWING_GRID)
        {
            ...
            Grid.draw(canvas, transform, p0, p1, p2, p3, label, Color.WHITE);

        }
        else if (primitive == DrawingPrimitive.DRAWING_IMAGE)
        {

            canvas.setMatrix(transform);
            // Draw the foreground masked by the supplied alpha bitmap.
            canvas.drawBitmapMesh(bmp, 1, 1, new float[]{
                    p0.x,p0.y,
                    p3.x,p3.y,
                    p1.x,p1.y,
                    p2.x,p2.y
